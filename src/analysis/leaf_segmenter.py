...
class LeafSegmenter:
    ...
    def request_stop(self):
        ...
        print("[LEAF SEGMENTER V6] STOP requested.", flush=True)
    ...
    def _check_stop(self):
        if self._stop_event.is_set():
            raise SegmentationStopped("Segmentation stopped by user.")
    ...
    def _tile_positions(
        ...
        if len(tiles) > self.max_tiles:
            ...
            selected = [tiles[min(int(i * step), len(tiles) - 1)] for i in range(self.max_tiles)]
            unique = []
            seen = set()
    ...
    # ================================================================
    # CLUMP SPLITTING HELPERS
    # ================================================================

    def _split_candidate_if_needed(self, candidate, image):
        self._check_stop()
        h, w = image.shape[:2]
        if not self._is_suspicious_clump(candidate, w, h):
            return [candidate]
        points = self._clump_points(candidate, image)
        if len(points) < 2:
            return [candidate]
        pieces = self._watershed_split(candidate["mask"], points)
        if len(pieces) < 2:
            return [candidate]
        result = []
        for index, piece in enumerate(pieces):
            self._check_stop()
            if not self._is_reasonable_leaf(piece, image.shape):
                continue
            ...
            result.append(child)
        return result if len(result) >= 2 else [candidate]

    def _remove_explained_parent_clumps(self, candidates):
        if len(candidates) < 3:
            return candidates
        ordered = sorted(candidates, key=lambda c: c.get("area", 0), reverse=True)
        removed = set()
        for i, parent in enumerate(ordered):
            self._check_stop()
            if parent.get("area", 0) < self.clump_min_area:
                continue
            parent_mask = parent["mask"] > 0
            parent_area = max(1, int(np.count_nonzero(parent_mask)))
            coverage_mask = np.zeros_like(parent["mask"], dtype=np.uint8)
            child_count = 0
            for j, child in enumerate(ordered):
                if i == j or child.get("area", 0) >= parent.get("area", 0):
                    continue
                if self.bbox_iou(parent["bbox"], child["bbox"]) <= 0:
                    continue
                if self.mask_containment(child["mask"], parent["mask"]) >= 0.70:
                    coverage_mask[child["mask"] > 0] = 255
                    child_count += 1
                if child_count >= self.parent_min_children:
                    coverage = np.count_nonzero((coverage_mask > 0) & parent_mask) / float(parent_area)
                    if coverage >= self.parent_coverage_threshold:
                        removed.add(id(parent))
                        break
        return [c for c in candidates if id(c) not in removed]
